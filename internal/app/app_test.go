...
func TestRun_Completion(t *testing.T) {
	...
	t.Run("bash", func(t *testing.T) {
		...
		// and on every subcommand -- eight word lists (rgitSubcommands, and
		// each of diff/commit/blame/log/languages/doctor/completion's own
		// flags), so eight occurrences of the pair in the order completion
		// offers them.
		qt.Assert(t, qt.Equals(strings.Count(stdout, "-h --help"), 8))
