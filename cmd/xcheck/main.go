...
//
// Every wired server must be reachable for a run to mean anything. Where a
// server is not already installed, put it in a scratch prefix and on PATH for
// the run alone rather than in a PATH directory, which would silently swap
// the tool the rest of the session uses:
//
...
//
// Python was the grammar this went unrun against longest, and running it is
// what turned pyright's binding-line convention from an assumption into
// lang_python.go's own trimDeclOnlyEnd.
package main
