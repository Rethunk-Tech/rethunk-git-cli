...
column only partly carries.** An untracked file with a supported grammar
reports ordinary `MOD` rows like any tracked change, via per-symbol
attribution; only a binary or ungrammared untracked file still surfaces
`UNTRACKED`, distinguishing "never committed" from "changed since `HEAD`"
for the files where a `MOD` row cannot carry that distinction. A
